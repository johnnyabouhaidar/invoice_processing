...
def process_invoice(filepath,outputpath):
    ...
    columnnames = ['Inv No', 'Date','PO Numbers']
    header_df = pd.DataFrame(columns=columnnames)

    header_fields= [[(1162, 297, 1364, 360),'Inv No'],[(66, 295, 212, 363),'Date'],[(63, 568, 1500, 665),'PO Numbers']]
    for field in header_fields:
        cropped = images[0].crop(field[0])
        if field[1]!="Date" and "PO" not in field[1]:
